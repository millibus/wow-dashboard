...
// generated-at.json, and scans raw file text for credential-shaped strings.
...
function validateMember(file, m, i) {
...
function validateGuildFile(file, data) {
  ...
  data.members.forEach((m, i) => validateMember(file, m, i));
}

function validateRaidFile(file, data) {
...
function validateCollectionsFile(file, data) {
...
function validateDir(dir) {
  ...
  if (!names.some(n => n.startsWith('guild-'))) errors.push(`${dir}: no guild-*.json files found`);
  ...
  for (const name of names) {
    ...
    if (name === 'generated-at.json') {
      ...
      validateGuildFile(name, data);
    } else if (name.startsWith('raid-')) {
      validateRaidFile(name, data);
    } else if (name.startsWith('collections-')) {
      validateCollectionsFile(name, data);
    }
  }
  return errors;
}
